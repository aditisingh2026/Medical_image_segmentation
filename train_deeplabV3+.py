# ============================================================
# train_deeplab.py — Polyp Segmentation Training (DeepLabV3+)
#
# Mirrors train.py (UNet++) exactly in:
#   - image size, split, augmentations, loss, optimizer, scheduler
# Differs only in:
#   - model architecture (DeepLabV3+ via segmentation_models_pytorch)
#   - no deep supervision (DeepLabV3+ has a single output head)
#
# This keeps the comparison fair: the ONLY thing that should
# explain a performance difference between this run and the
# UNet++ run is the architecture itself
# ------------------------------------------------------------
# ============================================================
...
# ============================================================
# TRAIN FUNCTION  (no deep supervision branch needed)
# ============================================================
...
# ============================================================
# DICE LOSS  (identical to train.py)
# ============================================================
...
if __name__ == "__main__":
    ...
    log_path   = config["log_path"].replace(".csv", "_deeplab.csv")
    mask_path  = config["mask_path"]
    image_path = config["image_path"]
    model_path = config["model_path"].replace(".pth", "_deeplab.pth")
    ckpt_path  = model_path.replace(".pth", "_checkpoint.pth")   # full resume state

    image_size = 384       # SAME as UNet++ run
    batch_size = config["batch_size"]
    lr = 1e-4               # SAME as UNet++ run

    # ---------------- DATA ----------------
    img_ids = glob(os.path.join(image_path, "*" + image_ext))
    img_ids = [os.path.splitext(os.path.basename(p))[0] for p in img_ids]

    # SAME split logic + SAME random_state -> identical train/val sets
    ...
    # ---------------- MODEL ----------------
    # encoder_weights=None -> trained from scratch, same starting
    # condition as the from-scratch UNet++ (fair comparison).
